...
TEST_CASE( "member operator overloading" ) {
    ...
    SUBCASE( "*=" ) { //NEW SUBCASE HW
        Vector3D a{1, 0, 0}, b{5, 0.005, 5};
        CHECK((a * 8) == Vector3D{8, 0, 0});
        CHECK((a * 0.5) == Vector3D{0.5, 0, 0});
        CHECK((a * b) == Vector3D{5, 0, 0});
        CHECK((b * a) == Vector3D{5, 0, 0});
    }
}
